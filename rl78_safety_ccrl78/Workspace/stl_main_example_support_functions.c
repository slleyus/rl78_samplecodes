...
/* macros */
#define IDBU_TB
#define hw_peripheral_crc_test_enabled
...
void indicate_test_result (char resultBool)
{
  ...
  result = resultBool;          /* Save result values for LED Pass / Fail selection */
      ...
      {
			P6_bit.no2 =  0U;     /* Test Pass Led1 ON */
			P6_bit.no3 = 1U;      /* Test Pass Led2 Off */
      }
      else                      /* Test failed */
      {          
        if (result == 1U)       /* Normal Test failure */
        {
			P6_bit.no2 = 1U;    /* Test failed Led1 Off */
			P6_bit.no3 = 0U;    /* Test failed Led2 On */
        }
        else                    /* Result = 0x02 => No reference signal for Clock test modules */
        {
			P6_bit.no2 = 0U;      /* Led1 On */
			P6_bit.no3 = 0U;      /* Led2 On */
        }
      }
	...
	P6_bit.no2 = 1U;            /* Turn both LED's off */
	P6_bit.no3 = 1U;
#else
