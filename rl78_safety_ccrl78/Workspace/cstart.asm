...
march_c_initial_test_enabled .EQU 1
march_x_initial_test_enabled .EQU 1
...
$ENDIF
    ...
    .EXTERN	_indicate_test_result
    .EXTERN	_LED_Display_Time_Delay

        .PUBLIC  _stl_RL78_march_c_initial_finish 
$IF(march_c_initial_test_enabled)
        .EXTERN  _stl_RL78_march_c_initial
$ENDIF
        .PUBLIC  _stl_RL78_march_x_initial_finish
$IF(march_x_initial_test_enabled)
        .EXTERN  _stl_RL78_march_x_initial
$ENDIF
...
$IF(march_c_initial_test_enabled)
;~~~~~~~startaddress of March C- test in register AX
       movw  AX,#0xF9D0
;~~~~~~~length of March C- test in register BC~~~~~~~       
       movw  BC,#0x0310
;~~~~~~~branche to march c- test ~~~~~~~~~~~~~~~~~~~~      
        BR    !!_stl_RL78_march_c_initial
;~~~~~~~testresult of March C- test in Register A -- 0x00 > no failure; 0x01 > failure        
_stl_RL78_march_c_initial_finish:
        MOV   0xFFEE8, A        
$ELSE
_stl_RL78_march_c_initial_finish:
$ENDIF
;~~~~~~~branche to march-x test ~~~~~~~~~~~~~~~~~~~~ 
$IF(march_x_initial_test_enabled)
;~~~~~~~startaddress of March X- test in register AX
        movw  AX,#0xF9D0
;~~~~~~~length of March X- test in register BC~~~~~~~       
        movw  BC,#0x0310
        BR    !!_stl_RL78_march_x_initial
;~~~~~~~testresult of March C- test in Register A -- 0x00 > no failure; 0x01 > failure
_stl_RL78_march_x_initial_finish:
        MOV   0xFFEE9, A
$ELSE
_stl_RL78_march_x_initial_finish:
$ENDIF
...
$IF(march_c_initial_test_enabled)
; indicate test result initial march-c test
        MOV     A,  0xFFEE8 
        CALL    !!_indicate_test_result       
$ENDIF
        CALL    !!_LED_Display_Time_Delay
$IF(march_x_initial_test_enabled)
; indicate test result initial march-x test        
        MOV     A,  0xFFEE9
        CALL    !!_indicate_test_result 
$ENDIF
;  end of inclusion for 60730 test stuff
